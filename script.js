...
window.addEventListener("click",e=>{ if(e.target.id==="closeModal"||e.target.id==="modal") modal.classList.add("hidden"); if(e.target.id === "addCart" && selectedProduct){   const link = selectedProduct.link || selectedProduct.affiliate_link || selectedProduct.url;    if(!link){     alert("Link affiliate produk belum diisi.");     return;   }    window.open(link, "_blank"); }});
